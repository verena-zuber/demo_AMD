...
There are two missing values in the AMD data, let's remove them, which reduces the sample size to n=148.
```{r message=FALSE, warning=FALSE}
remove.na=which(is.na(Challenge_dat$beta_amd)==TRUE)
Challenge_dat.na_rm = Challenge_dat[-remove.na,]
```
...
pmat_in = Challenge_dat.na_rm[,150:267]
```
